...
int main(int argc, char** argv)
{
	...
	for (int j = 0; j < NY; j++)
		x1[j] = 1.0f;// distribution(engine);

	for (int i = 0; i < NX; i++) {
		x2[i] = 2.0f;// distribution(engine);
		for (int j = 0; j < NY; j++)
			A[i*NY + j] = 1.0f;// distribution(engine);
	}
	...
	// BICG_BATCH is used only in fused kernel
	auto fused = [](std::vector<size_t> vector) {return vector.at(0) == 1 || (vector.at(0) == 0 && vector.at(1) == 1); };
	tuner.addConstraint(kernelId, fused, std::vector<std::string>{"FUSED", "BICG_BATCH"});
