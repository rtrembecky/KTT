...
//#endif
	...
	if (ty == 0) // first row of workgroup
		s_x1[tx] = x1[bx * TILE + tx]; // load x1 at [0-127]*32+[0-31] = [0-4064]+[0-31]
#endif // USE_SHARED_VECTOR1

#pragma unroll 1
	for (int i = ROWS_PROCESSED*by; i < ROWS_PROCESSED*(by + 1); i += TILE) { //i = n*by = 256*[0-15] = [0-3840]; i <= [224-4064];  8 iterations,  by = 0: i=0,32,64,96,128,160,192,224
...
#endif // USE_SHARED_VECTOR2
#if USE_SHARED_MATRIX == 1 || USE_SHARED_VECTOR2 == 1
		barrier(CLK_LOCAL_MEM_FENCE);
#endif
// multiply x2
...
#pragma unroll
#endif
		for (int j = 0; j < TILE; j += BICG_STEP) { // STEP is 4 for BATCH 8, means 8 iterations for j: 0,4,8,12,16,20,24,28
#if USE_SHARED_MATRIX == 1
			s_A[ty + j][tx] = A[(i + ty + j)*m + bx * TILE + tx]; // A at ([0-4064]+[0-3]+[0-28])*4096 + [0-127]*32 + [0-31], loaded to s_A at row [0-3]+[0-28] and column [0-31]
	#if USE_SHARED_VECTOR2 == 1
...
#pragma unroll
	...
	#if USE_SHARED_MATRIX == 1
			y1[i + tx + bx*ROWS_PROCESSED*gy] = tmp + s_A[tx][1];
	#else
			y1[i + tx + bx*ROWS_PROCESSED*gy] = tmp + A[(i + tx)*m + bx * TILE + 1];
	#endif // USE_SHARED_MATRIX
#endif // ATOMICS
	}
	...
	// s_A[ty][tx] = A[(ROWS_PROCESSED*by + ty)*m + bx * TILE + tx];
	// A[(i + tx)*m + bx * TILE + ty] = tmp = tmp + A[(i + tx)*m + bx * TILE + ty + TILE / 32];
...
#else // TODO: reduce in manipulator
	#if USE_SHARED_REDUCTION == 1
		y2[bx * TILE + tx + bx*ROWS_PROCESSED*gy] = l_sum + s_A[1][tx];
	#else
		y2[bx * TILE + tx + bx*ROWS_PROCESSED*gy] = l_sum + A[(ROWS_PROCESSED*by + ROWS_PROCESSED - TILE + 1)*m + bx * TILE + tx];
	#endif // USE_SHARED_REDUCTION
#endif // ATOMICS
}
